    ...
    const derivedAtom = memo1(() => {
        ...
        return atom(
            (get, { setSelf }) => {
                ...
                // if we have a cached value, return it early
                if (cache.has(promise)) {
                    lastValue = cache.get(promise) as Awaited<Value>;
                    return lastValue;
                }
                ...
                // handle if the handler immediately runs
                if (cache.has(promise)) {
                    lastValue = cache.get(promise) as Awaited<Value>;
                    return lastValue;
                }
